...
if has('nvim') || has('termguicolors')
  set termguicolors                             " Use proper colors in the terminal
endif
...
function! AddDependency()
    ...
    exec 'normal gg/construct%i' . dependency . ' $' . variable .
                \ 'o$this->' . dependency . ' = ' . variable .';'
endfunction

function! AddToConstructor()
    let b:variable = input('$this->')
    let b:value = input('= ')

    " Detect if a construct exists
    call cursor(1, 1)
    if search('__construct') == 0
        exec 'normal /classo' .
                    \ 'public function __construct()o{liddjO'
    endif

    exec 'normal gg/construct%o$this->' . b:variable . ' = ' . b:value . ';'
    exec 'normal kkkoPUBLIC/PRIVATE/PROTECTED $' . b:variable . ';o'
    exec 'normal kvlllll'
    silent exec 'MultipleCursorsFind PUBLIC/PRIVATE/PROTECTED'
endfunction

function! InitialiseConstructor()
    call cursor(1,1)

    " Add a constructor if we don't have one
    if search('__construct') == 0
        let b:filled_cons = 0
        exec 'normal /classo' .
    ...
    " If the constructor is filled then add a comma after the property
    if b:filled_cons == 1
        exec 'normal li, '
    endif
    ...
    if ! exists(b:filled_cons)
        exec 'normal li'
    endif

    silent exec 'MultipleCursorsFind PROPERTY'
endfunction
" }}}
" Mappings {{{
nnoremap <leader>ad :call AddToConstructor()<CR>
nnoremap <leader>ic :call InitialiseConstructor()<CR>
" }}}
" }}}
" PHP - Die and Dump {{{
" Functions {{{
function! InsertDisableExceptionHandling()
    let code = '$this->disableExceptionHandling();'
    exec 'normal [mo' . code
endfunction

" Uses Tim Pope's awesome Vim-Surround (the S in the method)
" Use range in the method to execute the visual command once!
function! DieAndDump() range
    " Remove the last semi-colon
    silent! '<,'>s/;\{1}$//
    " Wrap in a dd
    exec 'normal gvS(iddjj%a;'
endfunction
" }}}
" Mappings {{{
nnoremap <leader>de :call InsertDisableExceptionHandling()<CR>
vnoremap <leader>dd :call DieAndDump()<CR>
" }}}
" }}}
" PHP - Formatting Files {{{
" Function {{{
function! FormatPHPFile()

    let l:default_level = '@PSR2'
    let l:path_to_phpcsfixer = '/Users/Oli/.composer/vendor/bin/php-cs-fixer'

    let l:path_to_file = fnameescape(expand('%:p'))
    let l:path_to_config_file = fnameescape(ProjectRootGuess() . '/.php_cs')
    let l:php_cs_fixer_command = 'php '.l:path_to_phpcsfixer.' fix '.l:path_to_file

    if (&filetype != 'php')
        echo "The file must be a PHP file."
        return 1
    endif

    if (! filereadable(l:path_to_phpcsfixer))
        echo "Cannot find PHP-CS-Fixer. Exiting!"
        return 1
    endif

    if (filereadable(l:path_to_config_file))
        let l:command = l:php_cs_fixer_command . ' --config=' . l:path_to_config_file
    else
        let l:command = l:php_cs_fixer_command . ' --rules=' . l:default_level
    endif

    " Run the command and automatically edit the file suppressing the W11 error
    execute ':w'
    ...
    " Catch errors
    let errors = 0
    for word in split(l:output, ' ')
        if (match(tolower(word), 'error') != -1 || match(tolower(word), 'invalid') != -1)
            let errors = 1
        endif
    endfor

    " Thanks to stephpy/vim-php-cs-fixer for the nice error output
    if (v:shell_error > 1 || errors > 0)
        echohl Error | echo "Command:\n" . l:command . "\n\nOutput:\n" . l:output | echohl None
        return 1
    end
...
" }}}
" Mapping {{{
nnoremap <leader>p :call FormatPHPFile()<CR>
" }}}
" }}}
" PHP - Laravel {{{
" Functions {{{
function! ClassLookup()
    ...
    let classes = {
                ...
                \       'Events': 'Events/Dispatcher.php',
                \       'File': 'Filesystem/Filesystem.php',
                \       'Request': 'Http/Request.php',
                \       'Validator': 'Validation/Validator.php'
                \   }

    execute ':edit ' . ProjectRootGuess() . '/vendor/laravel/framework/src/Illuminate/' . classes[class]
endfunction
" }}}
" Mappings {{{
nmap <leader>lf :call ClassLookup()<cr>
" }}}
" }}}
" PHP - Syntax {{{
